...
export class DiaristaIndiceService implements DiaristaServiceSelecao {
  ...
  async selecionarMelhorDiarista(diaria: Diaria): Promise<UsuarioApi> {
    ...
    return await this.calcularMelhorCondidato(diaria, diaria.cep);
  }

  private validatorDiaria(diaria: Diaria) {
    if (diaria.candidatos.length <= 0) {
      throw new BadRequestException(
        'A lista de candidatos não pode estar vazia',
      );
    }
  }

  private async calcularMelhorCondidato(
    diaria: Diaria,
    destino: string,
  ): Promise<UsuarioApi> {
    let maiorIndice = 0;
    ...
    for (let i = 0; i < diaria.candidatos.length; i++) {
      const indiceCandidatoAtual = await this.calcularIndice(
        diaria.candidatos[i],
        destino,
      );
