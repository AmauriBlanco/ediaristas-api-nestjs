...
export class AvaliacaoValidator {
  ...
  private async validarAvaliador(avaliacao: Avaliacao) {
    ...
    if (avaliacoes.length > 0) {
      throw new BadRequestException('O Usuário ja avaliou essa diária');
    }
  }

  private validarDataAtendimento(avaliacao: Avaliacao) {
  ...
  private validarUsuarioDiaria(usuarioLogado: UsuarioApi, diaria: Diaria) {
    if (!diaria.diarista) {
      throw new BadRequestException('Diária não esta apta para avaliação');
    }
    if (
